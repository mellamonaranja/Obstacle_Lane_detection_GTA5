...
class LANE_DETECTION:
    ...
    def calc_perspective(self, lane_start=[0.25, 0.75]):
        ...
        if not hasattr(self, 'vanishing_point') or self.vanishing_point is None:
            ...
            x_threshold = 1.0 * self.img_dimensions[1]  # Adjusted threshold value
...
if __name__ == "__main__":
    video_reader = cv2.VideoCapture("./track_trimmed.mp4")
    ...
    video_writer = cv2.VideoWriter('./output_track_trimmed.mp4', fourcc, fps, (frame_w, frame_h))
    ...
    while True:
        ...
        ld = LANE_DETECTION(image, fps, 
                            ...
                            lum_factor=110,
                            lane_start=[0.2,0.5])
        # Assuming LANE_DETECTION or another method returns the processed frame
        processed_frame = image  # Replace this with the actual processed frame

        # Write the processed frame to the output video
        video_writer.write(processed_frame)

    # Release both video reader and writer
    video_reader.release()
    video_writer.release()
    print("Output video has been written to './output.avi'")